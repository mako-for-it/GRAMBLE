...
def parse_arguments():
    ...
    parser.add_argument(
        "--output_dir",
        default="../checkpoints/elc_bert_normalized",
        type=str,
    ...
    parser.add_argument(
        "--wandb_name", type=str, default="ELC BERT Normalized", help="WANDB run name."
    )
    parser.add_argument(
...
def setup_training(args):
    ...
    if is_main_process():
        ...
        print(
            f"Training for {args.max_steps:,} \
            steps with {get_world_size()} GPUs"
        )
        print(
            f"In total, the model will be trained on 'steps'\
            ({args.max_steps:,}) x 'GPUs'({get_world_size()}) x 'batch_size'\
            ({args.batch_size:,}) x 'seq_len'({args.seq_length:,}) = \
...
def training_epoch(
    ...
    train_dataloader = create_train_dataloader(data, args, seed)
