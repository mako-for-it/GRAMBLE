...
class SpanMaskingStrategy:
    ...
    def __call__(self, tokens):
        ...
        n_masked = torch.binomial(
            (tokens >= self.n_special_tokens).float().sum(dim=0, keepdim=True),
            torch.FloatTensor([self.mask_p])
        ).item()
        ...
        while n_masked > mask.long().sum() and counter > 0:
            ...
            offset = torch.randint(
                -(span_length - 1),
                tokens.size(0) + span_length,
                []
            ).item()
            sub_mask = torch.zeros_like(tokens, dtype=torch.bool)
            sub_mask[max(0, offset):min(mask.size(0)-1,
                     offset + span_length)] = True
...
class Dataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        target_seq_length = self.seq_length - 2 \
            if self.rand() > self.short_p \
            else self.randint(1, self.seq_length - 2)
    ...
    def show_random_item(self):
        ...
        print(' '.join(self.tokenizer.id_to_token(i) for i in inputs),
              flush=True)
        print(' '.join(self.tokenizer.id_to_token(o)
                       if o >= 0 else "-1" for o in outputs),
              flush=True)
